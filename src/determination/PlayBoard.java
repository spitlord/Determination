...
// Lightbulbs cannot be moved, but they can change states;
 ...
 public class PlayBoard extends Board {
     ...
     public void setColorScheme(String name) {
        ...
        PlayBoard.colorScheme = cs.getScheme(name);
           ...
           if (modular) {
                for (int y = row - dimention; y <= row + dimention; y++) {
                    for (int x = column - dimention; x <= column + dimention; x++) {
                        if (y < 0){
                            if (x < 0) {
                                blocks [PlayBoard.ROWS + y][PlayBoard.COLUMNS + x].switchColor();  
                            }
                            else {
                                blocks [PlayBoard.ROWS + y][x % PlayBoard.COLUMNS].switchColor();  
                            }
                        }
                        else {
                            if (x < 0) {
                                 blocks[y % PlayBoard.ROWS ][PlayBoard.COLUMNS + x].switchColor();  
                            }
                            else {
                                 blocks[y % PlayBoard.ROWS ][x % PlayBoard.COLUMNS].switchColor();  
                            }
                        }
                    }
               } 
               ...
               upperConstraint = (row + dimention < PlayBoard.ROWS)? row + dimention : PlayBoard.ROWS - 1;
               leftConstraint  = (column - dimention > 0) ? column - dimention : 0;
               rightConsraint  = (column + dimention < PlayBoard.COLUMNS) ? column + dimention : PlayBoard.COLUMNS - 1;
